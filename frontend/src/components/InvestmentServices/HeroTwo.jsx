...
export default function HeroTwo({ id, align }) {
      ...
      <Head>
        ...
        <meta
          ...
          content={
            ...
            'Explore our service offerings tailored to your financial goals.'
          }
        />
        <meta property="og:title" content={heroSection?.header || 'Wall Street Jr Services'} />
        <meta property="og:description" content={heroSection?.description || ''} />
        {heroSection?.thumbImg && (
          <meta property="og:image" content={heroSection.thumbImg} />
        )}
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={heroSection?.header || 'Wall Street Jr Services'} />
        <meta name="twitter:description" content={heroSection?.description || ''} />
        {heroSection?.thumbImg && (
          <meta name="twitter:image" content={heroSection.thumbImg} />
        )}
      </Head>

      {/* Extra BG on right side (desktop only) */}
      <div
        className={`absolute top-0 right-0 md:w-[100%] w-full h-full bg-no-repeat md:bg-${align} md:bg-right bg-contain z-0`}
        style={{
          backgroundImage: `url(${heroSection.bg})`,
          opacity: 0.6,
        }}
      />
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-12 pt-16">
          ...
          <div className="space-y-6 max-w-2xl">
            <div className="lg:pt-12 space-y-6">
              <div className="text-white text-sm md:text-base flex items-center space-x-2 py-10">
                <p className="font-light leading-snug pt-2 space-y-6">
                  {heroSection?.header?.split('?').map((line, index) => (
              ...
              <p className="text-white text-base md:text-[16px]">
                {heroSection?.description}
              </p>

              <div className="flex flex-wrap gap-4 items-center md:text-base text-sm py-6">
                <AnimatedButton>
                  Get Started Today
          ...
          <div className="flex flex-col justify-end h-full md:min-h-[550px] ">
            <div className="bg-gradient-to-r from-[#B9833E1A] to-[#BA833CB3] rounded-xl p-[1px] max-w-[448px] w-full ml-auto">              <div className="bg-[#0B1F2D] rounded-xl p-6 md:p-8">
              <h3 className="text-lg font-semibold mb-2 text-white">Contact a Specialist</h3>
